...
const inp   = "w-full h-11 dark:bg-white/5 bg-gray-50 dark:border-white/10 border border-gray-200 focus:dark:border-[#FFD100]/50 focus:border-[#FFD100] rounded-xl px-4 dark:text-white text-gray-900 font-bold text-sm outline-none transition-colors";
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={save}
          className="bg-[#FFD100] text-black font-bold text-sm px-5 py-2.5 rounded-xl hover:bg-[#E6BC00] transition-all">
      ...
      {prices.map(t => (
        <div key={t.id} className={`${card} p-5 space-y-4 shadow-sm`}>
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl bg-[#FFD100]/10 border border-[#FFD100]/20 flex items-center justify-center`}>
              <span className="text-[#FFD100] font-bold text-xs">{t.cap}</span>
            </div>
            ...
            <div className="text-right">
              ...
              <p className="text-[#FFD100] font-bold text-lg">{(t.base + demoKm * t.perKm).toLocaleString()} so&apos;m</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="range" min={10000} max={200000} step={5000} value={t.base}
                ...
                className="w-full mt-2 accent-[#FFD100]" />
            </div>
            <div>
      ...
      <div className={`${card} p-5 shadow-sm`}>
        ...
        <input type="range" min={1} max={50} step={1} value={demoKm}
          onChange={e => setDemoKm(+e.target.value)} className="w-full accent-[#FFD100]" />
        <div className="grid grid-cols-3 gap-3 mt-4">
          {prices.map(t => (
            <div key={t.id} className={`text-center dark:border-white/8 border border-gray-100 rounded-xl py-3`}>
              ...
              <p className="text-[#FFD100] font-bold">{(t.base + demoKm * t.perKm).toLocaleString()}</p>
            </div>
          ))}
        </div>
      </div>
