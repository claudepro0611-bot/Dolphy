  ...
  return (
    ...
    >
      ...
      <motion.div variants={fade} className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-2xl bg-[#C8F135]/15 border border-[#C8F135]/20 flex items-center justify-center flex-shrink-0">
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
            <circle cx="14" cy="10" r="5" stroke="#C8F135" strokeWidth="1.6"/>
            <path d="M4 26c0-5.523 4.477-9 10-9s10 3.477 10 9" stroke="#C8F135" strokeWidth="1.6" strokeLinecap="round"/>
          </svg>
        </div>
        <div>
      ...
      <motion.div variants={fade} className="grid grid-cols-3 gap-3">
        {STATS.map(s => (
          <div key={s.label} className="bg-gray-100 dark:bg-white/[0.04] border border-gray-200 dark:border-white/8 rounded-2xl p-4 text-center">
            <p className="text-[#C8F135] font-bold text-2xl leading-none">{s.value}</p>
      ...
      <motion.div variants={fade}
        ...
        <div>
          ...
          <p className="text-[#C8F135] font-bold text-2xl">
            {(MOCK_USER.spent / 1000).toFixed(0)}K
            <span className="text-sm font-normal text-gray-500 dark:text-white/40 ml-1">so&apos;m</span>
          </p>
        </div>
        <div className="w-12 h-12 rounded-2xl bg-[#C8F135]/10 flex items-center justify-center">
          <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
            <path d="M11 3v16M7.5 7H13a3 3 0 0 1 0 6H8a3 3 0 0 0 0 6H15" stroke="#C8F135" strokeWidth="1.6" strokeLinecap="round"/>
          </svg>
        </div>
      </motion.div>
